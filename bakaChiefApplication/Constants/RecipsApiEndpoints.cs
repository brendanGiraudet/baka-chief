...
public static class RecipsApiEndpoints
{
    ...
    public static string GetRecipsByNamePathUrl(int top, int skip, string name) => $"{GetRecipsPathUrl(top,skip)}&$filter=startswith(tolower(name), '{name.ToLower()}') eq true&$expand=RecipIngredients/Ingredient";
